...
MORDRED LIBRARY:
- Calculates 1800+ descriptors automatically
- We'll use ~100 most important ones for toxicity
"""
...
class DescriptorCalculator:
    """Calculates molecular descriptor for ML models"""
    ...
    def __init__(self):
        ...
        logger.info(f"DescriptorCalculator initialized with {len(self.descriptor_names)} descriptors")
    
    def _setup_calculator(self):
        """
        Setup Mordred calculator with selected descriptors.
        
        MORDRED HAS 1800+ DESCRIPTORS:
        - We pick the most important ~100 for toxicity
        - Focuses on: structure, electronic properties, topology
        """
        ...
        logger.debug(f"Calculator setup with descriptor families: AtomCount, BondCount, RingCount, TopologicalIndex, ConnectivityIndex")
    ...
    def calculate(self, mol: Chem.Mol, smiles: str) -> Dict[str, float]:
        ...
        RETURNS:
            Dictionary: {"descriptor_name": value, ...}
            
        EXAMPLE:
            {"nHeavyAtom": 3, "TopoPSA": 20.23, "BertzCT": 25.4, ...}
        """
        
        if mol is None:
            raise DescriptorCalculationError(f"Cannot calculate descriptors for None molecule: {smiles}")
        ...
        try:
            ...
            raise DescriptorCalculationError(f"Failed to calculate descriptors for {smiles}: {e}")
    ...
    def calculate_cached(self, smiles: str) -> Dict[str, float]:
        ...
        if mol is None:
            raise DescriptorCalculationError(f"Invalid SMILES for descriptor calculation: {smiles}")
    ...
    for smiles, name in test_molecules:
        try:
            ...
            # Show first 10 descriptors as examples
