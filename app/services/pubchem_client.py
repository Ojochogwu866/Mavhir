# toxicity-predictor/app/services/pubchem_client.py
"""
PubChem API client for chemical database lookups.

PubChem is NCBI's free chemical database with 100+ million compounds.
We use it to:
- Look up compounds by name/CAS number
...
class PubChemClient:
    ...
    - Rate limits: 5 requests/second, 400/minute
    - Free to use, no API key required
    - Returns JSON or XML (we use JSON)
    
    SEARCH TYPES:
    - By name: /compound/name/{name}/property/...
    - By CID: /compound/cid/{cid}/property/...
    - By SMILES: /compound/smiles/{smiles}/property/...
    """
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            'User-Agent': 'ToxicityPredictor/1.0 (https://github.com/your-repo)'
        })
        
        self._last_request_time = 0.0
        
        logger.info(f"PubChemClient initialized (timeout: {self.timeout}s, delay: {self.rate_delay}s)")
    ...
    def search_by_name(self, name: str) -> CompoundInfo:
        ...
        if not name:
            return CompoundInfo(
                cid=None, name=None, smiles=None, 
                molecular_formula=None, molecular_weight=None,
                synonyms=None, found=False
            )
        ...
        try:
            ...
            if cid is None:
                return CompoundInfo(
                    cid=None, name=name, smiles=None,
                    molecular_formula=None, molecular_weight=None,
                    synonyms=None, found=False
                )
    ...
    def _get_compound_properties(self, cid: int, query_name: Optional[str] = None) -> CompoundInfo:
        ...
        try:
            ...
            if 'PropertyTable' in data and 'Properties' in data['PropertyTable']:
                props = data['PropertyTable']['Properties'][0]  # First result
                ...
                return CompoundInfo(
                    ...
                    synonyms=None,  # Could fetch separately if needed
                    found=True
                )
            
            return CompoundInfo(
                cid=cid, name=query_name, smiles=None,
                molecular_formula=None, molecular_weight=None,
                synonyms=None, found=False
            )
    ...
    def _make_request(self, url: str) -> requests.Response:
        ...
        - We implement a simple delay between requests
        """
