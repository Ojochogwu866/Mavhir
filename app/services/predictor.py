...
from ..core.exceptions import ModelLoadError, ModelPredictionError, UnsupportedModelError
...
class ToxicityModel:
    ...
    def _load_model(self) -> BaseEstimator:
        ...
        try:
            ...
            if model_file.suffix == '.pkl':
                ...
                raise ModelLoadError(self.model_path, f"Unsupported file format: {model_file.suffix}")
	...
	def predict(self, descriptors: Dict[str, float]) -> PredictResult:
		"""Make toxicity prediction from molecular descriptors
		PARAMETERS:
            descriptors: Dictionary of descriptor name -> value
            
        RETURNS:
            Prediction Result with prediction, probability, confidence
		"""
		try:
	...
	def _prepare_features(self, descriptors: Dict[str, float]) -> np.ndarray:
        ...
        SOLUTION:
        - For now, use all descriptor values in alphabetical order
        - If ylou are reading this code base after forking, in production, it is advisable to save the features name with model
        """
        
        # Sort descriptors by name for consistency
    ...
    def _get_prediction_class(self, probability: float) -> PredictionClass:
        ...
        if self.endpoint == ToxicityEndpoint.AMES_MUTAGENICITY:
            return PredictionClass.MUTAGENIC if is_positive else PredictionClass.NON_MUTAGENIC
        elif self.endpoint == ToxicityEndpoint.CARCINOGENICITY:
            return PredictionClass.CARCINOGENIC if is_positive else PredictionClass.NON_CARCINOGENIC
        else:
            # Future endpoints
    ...
    def _get_confidence_level(self, probability: float) -> ConfidenceLevel:
        ...
        LOGIC:
        - High confidence: Very sure (prob < 0.25 or > 0.75)
        - Medium confidence: Somewhat sure (0.25-0.4 or 0.6-0.75)  
        - Low confidence: Uncertain (0.4-0.6)
        """
...
class ToxicityPredictor:
    ...
    ORCHESTRATES:
    - Multiple toxicity models (Ames, Carcinogenicity, etc.)
    - Batch predictions
    ...
    def predict(
        ...
        PARAMETERS:
            descriptors: Molecular descriptors from DescriptorCalculator
            smiles: SMILES string (for logging/errors)
            endpoints: Which endpoints to predict (default: all available)
            
        RETURNS:
            CompoundPredictions with all requested predictions
        """
        
        import time
        start_time = time.time()
        
        # Determine which endpoints to predict
        ...
        if unavailable:
            ...
            raise UnsupportedModelError(str(unavailable), [ep.value for ep in available])
        ...
        for endpoint in endpoints:
            try:
                ...
                # Could create a "failed" prediction result instead of skipping
                continue
    ...
    def predict_batch(
        ...
        for smiles, descriptors in compounds:
            try:
                ...
                results.append(CompoundPredictions(
                    smiles=smiles,
                    predictions={},
                    descriptors_used=0,
                    processing_time=0.0
...
def _test_predictor():
    ...
    try:
        ...
        print(f"Available endpoints: {[ep.value for ep in predictor.get_available_endpoints()]}")
        print()
