...
from rdkit.Chem import Descripors, rdMolDescriptors, Crippen, Lipinski
...
class MolecularProperties:
	...
	These properties are essential for:
		- Drug discovery (Lipinski's Rule of 5)
		- ADMET prediction 
...
class MolecularStandardizer:
    """YHandles molecular structure standardization usig RDKit"""
    
    def __init__(self):
        """Initialize standardization compoennts."""
        try:
            self.normalizer = rdMolStandardize.Normalizer()
            self.largest_fragment_chooser = rdMolStandardize.LargestFragmentChooser
            self.uncharger = rdMolStandardize.Uncahrger()
            self.tautomer_enumerator = rdMolStandardize.TautomerEnumerator()
            
            logger.debug('Molecule standadizer initiated successfully')
            
        except Exception as e:
            logger.error(f"Failed to initialize Molecule standardizer: {e}")
    ...
    def standardize(self, mol: Chem.mol, smiles: str) -> Chem.mol:
        ...
        if mol is None:
            raise MoleculeStandardizationError(smiles, "parse", "Input molecule is None")
        try:
            mol = self.normalizer.normalize(mol)
            if mol is None:
                raise MoleculeStandardizationError(smiles, "normalize", "Normalization returned None")
            
            mol - self.largest_Fragment_chooser.choose(mol)
            if mol is None:
                raise MoleculeStandardizationError(smiles, "fragment_selection", "Fragment selection returned  None")
            
            mol = self.uncharger.uncharge(mol)
            if mol is None:
                raise MoleculeStandardizationError(smiles, "neutralize", "Charge neutralization returned None")
            
            mol = self.tautomer_enumerator.Canonicalize(mol)
            if mol is None:
                raise MoleculeStandardizationError(smiles, "tautomer_canonicalization", "Tautomer canonicalization returned None")
            ...
            raise MoleculeStandardizationError(smiles, "unknown", f"Unexpected error: {e}")
        
    def quick_standardizae(self, mol: Chem.Mol, smiles: str) -> Chem.Mol:
        """
        ...
        try:
            ...
            if mol is None:
                raise MoleculeStandardizationError(smiles, "Quick_standardize", "Quick standardization failed")
            
            return mol
        
        except Exception as e:
            raise MoleculeStandardizationError(smiles, "quick_standardize", f"Quick standardization error: {e}")
...
class PropertyCalculator:
    ...
    These 8 properties are chosen because they:
    - Correlate strongly with ADMET properties
    - Are fast to calculate
    - Are interpretable by chemists
    - Are used in most pharmaceutical QSAR models
    """
    ...
    def calculate_properties(mol: Chem.Mol) -> MolecularProperties:
        ...
        - TPSA: Topological (2D), not 3D surface area
        - Others: Standard RDKit atom-based calculations
        """
        try:
    ...
    def assess_drug_likeness(properties: MolecularProperties) -> Dict[str, Any]:
        ...
        RULES IMPLEMENTED:
        1. Lipinski's Rule of 5 (oral bioavailability)
        2. Veber Rules (additional bioavailability criteria)
        """
        ...
        lipinski_passed = len(lipinski_violations) <= 1  # Allow 1 violation
        
        # Veber Rules evaluation
        veber_violations = []
        
        if properties.num_rotatable_bonds > 10:
            veber_violations.append(f"Rotatable bonds > 10 ({properties.num_rotatable_bonds})")
        
        if properties.tpsa > 140:
            veber_violations.append(f"TPSA > 140 ({properties.tpsa:.1f})")
        
        veber_passed = len(veber_violations) == 0  # No violations allowed
...
class ChemicalProcessor:
    ...
    def __init__(self):
        ...
        logger.info(f"ChemicalProcessor initialized (standardization: {self.enable_standardization})")
        
        def process_smiles(self, smiles: str, quick_mode: bool = false) -> ProcessedMolecule:
            """Process a single SMILES string through the full pipeline
            PARAMETERS:
				smiles: Input SMILES string
				quick_mode: Use fast standardization (skip tautomers)
            
            RETURNS:
                    ProcessedMolecule with results or error information
            """
            smiles = smiles.strip()
            ...
            try:
                ...
                return self._create_invalid_molecule(smiles, [f"Failed to generate canonical SMILES: {e}"])
            ...
            try:
                ...
                logger.warning(f"Molecular formula calculation failed for {smiles}: {e}")
            
            # Step 5: Calculate properties
            try:
                properties = self.property_calculator.calculate_properties(mol)
            except Exception as e:
                return self._create_invalid_molecule(smiles, [f"Property calculation failed: {e}"])
            ...
            return self._create_invalid_molecule(smiles, [f"Unexpected processing error: {e}"])
        
    def process_smiles_batch(self, smiles_list: List[str], quick_mode: bool = False) -> List[ProcessedMolecule]:
        """
        Process a batch of SMILES strings.
        PARAMETERS:
            smiles_list: List of SMILES strings to process
            quick_mode: Use fast standardization for all molecules
            
        """
        if not smiles_list:
            return []
        
        logger.info(f"Processing batch of {len(smiles_list)} SMILES (quick_mode: {quick_mode})")
        ...
        for i, smiles in enumerate(smiles_list):
            try:
                ...
                # This should never happen since process_smiles doesn't raise
                logger.error(f"Unexpected error in batch processing at index {i}: {e}")
                results.append(self._create_invalid_molecule(smiles, [f"Batch processing error: {e}"]))
                failed += 1
        
        success_rate = (successful / len(smiles_list)) * 100
        logger.info(f"Batch processing complete: {successful} successful, {failed} failed ({success_rate:.1f}% success rate)")
    ...
    def get_canonical_smiles(self, smiles: str) -> str:
        ...
        - Only does parsing and canonicalization
        - Skips property calculation and standardization
        - Useful for deduplication and database lookups
    ...
    def _create_invalid_molecule(self, smiles: str, errors: List[str]) -> ProcessedMolecule:
        """Create ProcessedMolecule for invalid input."""
        
        # Create empty properties for invalid molecules
...
def create_chemical_processor() -> ChemicalProcessor:
    """
    Factory function to create ChemicalProcessor. Useful for dependency injection and testing.
    """
...
def validate_smiles_simple(smiles: str) -> bool:
    ...
    FAST CHECK:
    - Only tests if RDKit can parse the SMILES
    - No standardization or property calculation
    - Useful for quick validation in APIs
    """
    try:
        mol = Chem.MolFromSmiles(smiles.strip())
    ...
    for smiles, description in test_cases:
        ...
        try:
            ...
            if result.is_valid:
                ...
                # Test drug-likeness
                drug_assessment = processor.property_calculator.assess_drug_likeness(result.properties)
