...
class MolecularProperties(BaseModel):
    ...
    num_heavy_atoms: int = Field(description="Number of heavy (non-hydrogen) atoms", ge=0)
...
class DrugLikenessAssessment(BaseModel):
    ...
    lipinski_violations: int = Field(description="Number of Lipinski Rule of 5 violations", ge=0, le=4)
    lipinski_passed: bool = Field(description="Passes Lipinski rules (≤1 violation allowed)")
    veber_violations: int = Field(description="Number of Veber rule violations", ge=0, le=2)
    ...
    violation_details: Optional[Dict[str, str]] = Field(None, description="Details of rule violations")
...
class PredictionResults(BaseModel):
    ...
    ames_mutagenicity: Optional[SinglePrediction] = Field(None, description="Ames mutagenicity prediction")
    carcinogenicity: Optional[SinglePrediction] = Field(None, description="Carcinogenicity prediction")
...
class SMILESPredictionRequest(BaseModel):
    ...
    endpoints: Optional[List[ToxicityEndpoint]] = Field(
        None, 
        description="Specific toxicity endpoints to predict (default: all)"
    )
    include_descriptors: bool = Field(
        default=False, 
        description="Include molecular descriptors in response"
    )
    include_properties: bool = Field(
        default=True, 
        description="Include molecular properties in response"
    )
    include_drug_likeness: bool = Field(
        default=True, 
        description="Include drug-likeness assessment"
    )
    ...
    def validate_smiles_format(cls, v):
        ...
        invalid_chars = set(v) - set("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789@+\\-[]()=#%/\\.")
        if invalid_chars:
            raise ValueError(f"SMILES contains invalid characters: {', '.join(sorted(invalid_chars))}")
...
class CompoundPredictionResponse(BaseModel):
    ...
    descriptors: Optional[Dict[str, float]] = Field(
		None,
		description="Molecular descriptors used by ML models"
	)
...
class BatchResultItem(BaseModel):
    ...
    result: Optional[CompoundPredictionResponse] = Field(None, description="Prediction result if successful")
...
class BatchPredictionSummary(BaseModel):
    ...
    total_compounds: int = Field(description="Total number of compounds processed", ge=0)
...
class ChemicalLookupResponse(BaseResponse):
    ...
    canonical_smiles: Optional[str] = Field(None, description="Canonical SMILES from database")
...
class ServiceStatus(BaseModel):
    ...
    status: Literal["healthy", "unhealthy", "unknown"] = Field(description="Service status")
